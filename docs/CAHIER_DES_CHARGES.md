...
## PARTIE 2 — Knowledge Base universelle — 🟡 DÉMARRÉ (11✅/0🟡/24⬜ sur 35, via les Étapes 2.1.1/2.1.2/2.1.3/2.1.4/2.1.5/2.1.6/2.1.7/2.1.8/2.1.9/2.1.10/2.1.11)
...
| ✅ Fait | 70 | 14.0% |
| 🟡 Partiel | 64 | 12.8% |
| ⬜ Non commencé | 366 | 73.2% |

**Complétion globale (/515, Partie 15 incluse en approximation)** :
- Strictement ✅ : **70/515 (~13.6%)**
- ✅ + 🟡 touchés d'une manière ou d'une autre : **134/515 (~26.0%)**
- Pondéré (✅=1, 🟡=0.5) : **~102.0/515 (~19.8%)** -- le chiffre le plus représentatif de l'avancement réel.

Mis à jour après Partie 2.1.11 (Import via Sitemap, 2026-09-03) :
...
3. **Partie 2 (Knowledge Base multi-format), le reste** -- le cœur
   ...
   (URLs/pages web) et 2.1.11 (Sitemap) livrés -- 2.1.10 a introduit sa
   propre vraie nouveauté (SSRF, route/colonne/tâche Celery dédiées),
   et 2.1.11 a construit dessus en pure réutilisation ("réutilisation
   sur réutilisation" : fetch SSRF-safe et pipeline par page tous deux
   inchangés depuis 2.1.10), sans introduire la moindre nouvelle
   colonne de base de données. Reste 2.1.12 à 2.1.19 (8 autres
