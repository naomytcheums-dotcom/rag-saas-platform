...
## PARTIE 2 — Knowledge Base universelle — 🟡 DÉMARRÉ (6✅/0🟡/29⬜ sur 35, via les Étapes 2.1.1/2.1.2/2.1.3/2.1.4/2.1.5/2.1.6)
...
| ✅ Fait | 65 | 13.0% |
| 🟡 Partiel | 64 | 12.8% |
| ⬜ Non commencé | 371 | 74.2% |

**Complétion globale (/515, Partie 15 incluse en approximation)** :
- Strictement ✅ : **65/515 (~12.6%)**
- ✅ + 🟡 touchés d'une manière ou d'une autre : **129/515 (~25.0%)**
- Pondéré (✅=1, 🟡=0.5) : **~97.0/515 (~18.8%)** -- le chiffre le plus représentatif de l'avancement réel.

Mis à jour après Partie 2.1.6 (Import de documents CSV, 2026-09-03) :
...
3. **Partie 2 (Knowledge Base multi-format), le reste** -- le cœur
   ...
   (PDF), 2.1.2 (DOCX), 2.1.3 (TXT), 2.1.4 (Markdown), 2.1.5 (HTML) et
   2.1.6 (CSV) livrés, même pipeline partagé -- reste 2.1.7 à 2.1.19
   (13 autres formats d'import) et toute la Partie 2.2 (gestion des
   documents : tags, versioning, réindexation, détection de doublons,
   sync). Gros chantier restant, à continuer de découper en sous-étapes.
