...
### 1.4 Domaine & White-label — 🟡 DÉMARRÉ (4✅/3🟡/3⬜ sur 10, via les Étapes 1.4.1/1.4.2/1.4.3 + réutilisation honnête de 1.3.9/1.3.10)
...
| ✅ Fait | 57 | 11.4% |
| 🟡 Partiel | 63 | 12.6% |
| ⬜ Non commencé | 380 | 76.0% |

**Complétion globale (/515, Partie 15 incluse en approximation)** :
- Strictement ✅ : **57/515 (~11.1%)**
- ✅ + 🟡 touchés d'une manière ou d'une autre : **120/515 (~23.3%)**
- Pondéré (✅=1, 🟡=0.5) : **~88.5/515 (~17.2%)** -- le chiffre le plus représentatif de l'avancement réel.

Mis à jour après Partie 1.4.3 (SSL auto Let's Encrypt, 2026-09-02) :
...
- Partie 1.4 (White-label), le reste -- démarrée (4✅/3🟡/3⬜ sur 10,
  voir le tableau détaillé ci-dessus) : 1.4.1 (custom domains), 1.4.2
  (instructions DNS bilingues) et 1.4.3 (SSL via un vrai client ACME,
  🟡 -- DNS-01 fonctionnel et vérifié contre le vrai staging Let's
  Encrypt, mais pas "auto" sans intégration API fournisseur DNS)
  livrés ; 1.4.7/1.4.8/1.4.10 déjà satisfaits en tout ou partie par
  1.3.9/1.3.10. Ce qui manque reste réel infrastructure (un
  reverse-proxy dynamique routant réellement le trafic par Host header,
  dont 1.4.1 ET 1.4.3 dépendent tous les deux pour qu'un domaine
  "actif" serve vraiment quelque chose), le job Celery de polling de
  1.4.4, 1.4.5/1.4.9 (email par domaine custom), et 1.4.6 (flag
  white-label + templates conditionnels)
