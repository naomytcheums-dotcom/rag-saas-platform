...
## PARTIE 2 — Knowledge Base universelle — 🟡 DÉMARRÉ (10✅/0🟡/25⬜ sur 35, via les Étapes 2.1.1/2.1.2/2.1.3/2.1.4/2.1.5/2.1.6/2.1.7/2.1.8/2.1.9/2.1.10)
...
| ✅ Fait | 69 | 13.8% |
| 🟡 Partiel | 64 | 12.8% |
| ⬜ Non commencé | 367 | 73.4% |

**Complétion globale (/515, Partie 15 incluse en approximation)** :
- Strictement ✅ : **69/515 (~13.4%)**
- ✅ + 🟡 touchés d'une manière ou d'une autre : **133/515 (~25.8%)**
- Pondéré (✅=1, 🟡=0.5) : **~101.0/515 (~19.6%)** -- le chiffre le plus représentatif de l'avancement réel.

Mis à jour après Partie 2.1.10 (Import de pages web par URL, 2026-09-03) :
...
3. **Partie 2 (Knowledge Base multi-format), le reste** -- le cœur
   ...
   2.1.6 (CSV), 2.1.7 (JSON), 2.1.8 (XML), 2.1.9 (EPUB) et 2.1.10
   (URLs/pages web) livrés -- 2.1.10 a introduit sa propre vraie
   nouveauté (SSRF, route/colonne/tâche Celery dédiées) tout en
   réutilisant intégralement le pipeline existant pour le traitement
   lui-même. Reste 2.1.11 à 2.1.19 (9 autres formats/sources d'import)
   et toute la Partie 2.2 (gestion des documents : tags, versioning,
