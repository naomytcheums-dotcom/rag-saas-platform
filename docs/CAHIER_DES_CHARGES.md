...
## PARTIE 2 — Knowledge Base universelle — 🟡 DÉMARRÉ (7✅/0🟡/28⬜ sur 35, via les Étapes 2.1.1/2.1.2/2.1.3/2.1.4/2.1.5/2.1.6/2.1.7)
...
| ✅ Fait | 66 | 13.2% |
| 🟡 Partiel | 64 | 12.8% |
| ⬜ Non commencé | 370 | 74.0% |

**Complétion globale (/515, Partie 15 incluse en approximation)** :
- Strictement ✅ : **66/515 (~12.8%)**
- ✅ + 🟡 touchés d'une manière ou d'une autre : **130/515 (~25.2%)**
- Pondéré (✅=1, 🟡=0.5) : **~98.0/515 (~19.0%)** -- le chiffre le plus représentatif de l'avancement réel.

Mis à jour après Partie 2.1.7 (Import de documents JSON, 2026-09-03) :
...
d'écrire le code de production : le parseur C tolère l'imbrication
bien au-delà de la limite de récursion Python par défaut (échec réel
à 2999 niveaux) ; une première implémentation récursive du calcul de
profondeur échouait dès 500 niveaux -- bien avant cette limite -- un
vrai bug corrigé en passant à une version itérative avant la livraison.
...
3. **Partie 2 (Knowledge Base multi-format), le reste** -- le cœur
   ...
   2.1.6 (CSV) et 2.1.7 (JSON) livrés, même pipeline partagé -- reste
   2.1.8 à 2.1.19 (12 autres formats d'import) et toute la Partie 2.2
   (gestion des documents : tags, versioning, réindexation, détection
   de doublons, sync). Gros chantier restant, à continuer de découper
   en sous-étapes.
