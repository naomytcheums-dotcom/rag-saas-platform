...
### 1.4 Domaine & White-label — 🟡 DÉMARRÉ (5✅/3🟡/2⬜ sur 10, via les Étapes 1.4.1/1.4.2/1.4.3/1.4.4 + réutilisation honnête de 1.3.9/1.3.10)
...
- Pondéré (✅=1, 🟡=0.5) : **~89.0/515 (~17.3%)** -- le chiffre le plus représentatif de l'avancement réel.

Mis à jour après Partie 1.4.4 (Vérification domaine périodique, 2026-09-02) :
...
- Partie 1.4 (White-label), le reste -- démarrée (5✅/3🟡/2⬜ sur 10,
  ...
  Encrypt, mais pas "auto" sans intégration API fournisseur DNS) et
  1.4.4 (vérification domaine périodique, job Celery de polling réel)
  ...
  "actif" serve vraiment quelque chose), 1.4.5/1.4.9 (email par domaine
  custom), et 1.4.6 (flag white-label + templates conditionnels)
