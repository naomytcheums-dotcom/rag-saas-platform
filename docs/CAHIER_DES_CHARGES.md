...
## PARTIE 2 — Knowledge Base universelle — 🟡 DÉMARRÉ (12✅/0🟡/23⬜ sur 35, via les Étapes 2.1.1/2.1.2/2.1.3/2.1.4/2.1.5/2.1.6/2.1.7/2.1.8/2.1.9/2.1.10/2.1.11/2.1.12)
...
| ✅ Fait | 71 | 14.2% |
| 🟡 Partiel | 64 | 12.8% |
| ⬜ Non commencé | 365 | 73.0% |

**Complétion globale (/515, Partie 15 incluse en approximation)** :
- Strictement ✅ : **71/515 (~13.8%)**
- ✅ + 🟡 touchés d'une manière ou d'une autre : **135/515 (~26.2%)**
- Pondéré (✅=1, 🟡=0.5) : **~103.0/515 (~20.0%)** -- le chiffre le plus représentatif de l'avancement réel.

Mis à jour après Partie 2.1.12 (Import depuis GitHub -- dépôts, 2026-09-03) :
...
3. **Partie 2 (Knowledge Base multi-format), le reste** -- le cœur
   ...
   (URLs/pages web), 2.1.11 (Sitemap) et 2.1.12 (GitHub -- dépôts)
   livrés -- 2.1.10 a introduit sa propre vraie nouveauté (SSRF,
   ...
   et 2.1.12 réutilise le même pipeline de traitement à une source et
   un modèle de menace différents (API GitHub REST sur un hôte fixe,
   pas de transport SSRF-safe nécessaire ; token serveur jamais
   transmis via Celery). Aucune des trois n'a introduit la moindre
   nouvelle colonne de base de données. Reste 2.1.13 à 2.1.19 (7 autres
   formats/sources d'import) et toute la Partie 2.2 (gestion des
   documents : tags, versioning,
