...
✅ **Partie 16 (ter) — Marketplace de plugins — ✅ COMPLET (scope honnête)** — voir [`docs/marketplace/PARTIE_16_TER_MARKETPLACE.md`](marketplace/PARTIE_16_TER_MARKETPLACE.md). Système réel : `Plugin`/`PluginInstallation`/`PluginReview`, validation stricte du manifest (schéma + whitelist de permissions fixe) et scan statique réel du code (patterns dangereux rejetés : `eval`, `exec`, `os.system`, `child_process`, etc.) avant toute publication. 16 endpoints réels (marketplace public, publication/installation org-scoped, modération admin). Frontend : une page consolidée `/dashboard/marketplace` (3 onglets). Vérifié en direct de bout en bout dans un vrai navigateur : publication réelle → upload S3 réel → statut `pending` → approbation admin réelle → visibilité marketplace → installation réelle → désinstallation. Un vrai bug trouvé et corrigé pendant cette vérification (colonne `updated_at` avec `onupdate` causant un crash `MissingGreenlet` après une UPDATE — corrigé par génération d'UUID en amont + `db.refresh()` après commit). Scope honnête et explicite sur le "sandbox" : aucun moteur d'exécution de plugins n'existe dans ce projet — la validation/scan statique sont réels, l'exécution réelle du code ne l'est pas (fabriquer un faux sandbox aurait été contraire à la discipline du projet).
