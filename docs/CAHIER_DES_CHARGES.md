...
### 1.4 Domaine & White-label — 🟡 DÉMARRÉ (4✅/2🟡/4⬜ sur 10, via les Étapes 1.4.1/1.4.2 + réutilisation honnête de 1.3.9/1.3.10)
...
- Pondéré (✅=1, 🟡=0.5) : **~88/515 (~17.1%)** -- le chiffre le plus représentatif de l'avancement réel.

Mis à jour après Partie 1.4.2 (Instructions DNS, 2026-09-02) :
...
- Partie 1.4 (White-label), le reste -- démarrée (4✅/2🟡/4⬜ sur 10,
  voir le tableau détaillé ci-dessus) : 1.4.1 (custom domains) et 1.4.2
  (instructions DNS bilingues) livrés ; 1.4.7/1.4.8/1.4.10 déjà
  satisfaits en tout ou partie par 1.3.9/1.3.10. Ce qui manque reste
  réel infrastructure (1.4.3 SSL auto/reverse-proxy dynamique, dont
  1.4.1 dépend pour qu'un domaine "actif" serve vraiment quelque
  chose), le job Celery de polling de 1.4.4, 1.4.5/1.4.9 (email par
  domaine custom), et 1.4.6 (flag white-label + templates conditionnels)
