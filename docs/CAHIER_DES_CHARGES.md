...
✅ **12.3 bis Pack IA inclus + BYOK (2026-09-17)** -- le système de crédits ci-dessus existait déjà mais n'était réellement débité nulle part ; corrigé : chaque appel LLM réel (`api/services/agent_orchestrator.py`) débite maintenant les crédits réels, calculés à partir du vrai nombre de tokens rapporté par le fournisseur (`chat_completion_with_usage`), via la table de conversion déjà existante (`api/security/credit_packs.py`). `Plan.monthly_credits_included` (nouvelle colonne, migration `0109`) + tâche Celery mensuelle idempotente (`grant_monthly_plan_credits`) accordent un pack de crédits inclus à chaque abonnement actif. BYOK (Bring Your Own Key, `api/models/organization_llm_config.py`, `api/services/llm_byok.py`) permet à une organisation de fournir sa propre clé (chiffrée avec le même mécanisme Fernet déjà utilisé pour les clés JWT/SSO) pour un fournisseur LLM -- ses appels avec cette clé ne consomment alors aucun crédit, facturés directement par le fournisseur. Voir `docs/features/AI_CREDITS.md` et `docs/features/BYOK.md`.
