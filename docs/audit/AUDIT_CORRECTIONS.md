...
|---|------------|------------|------|
| 1 | CI complete (pytest, pip-audit, tsc, eslint, vitest, npm audit) | `.github/workflows/ci.yml`, `docs/devops/CI.md` | Partiel -- chaque commande verifiee individuellement en local (109+26+3 tests backend, 77 tests frontend, tsc propre) ; le run GitHub reel n'a pas pu etre observe dans cette session. 132 erreurs de lint preexistantes decouvertes et rendues visibles sans bloquer (`continue-on-error`) |
...
| 7 | Accessibilite | `docs/ux/ACCESSIBILITY.md`, nouveaux composants (`LanguageSelector`, `LoadingState`) accessibles par construction | Partiel -- le flux chat (deja verifie couvert), les nouveaux composants ; le reste du dashboard (~30 pages) non audite exhaustivement |
...
| 13 | Bundle size frontend | -- | Non fait -- `npm run build` non execute dans cette session |
| 14 | Message "Chargement..." avec repli + retry | `frontend/components/LoadingState.tsx` (nouveau), applique a `dashboard/layout.tsx` et `chat/page.tsx` | OK pour ces 2 pages -- type-check et lint propres. 19 autres pages avec un "Chargement..." statique non converties (risque juge trop eleve pour le temps disponible) |
...
| 19 | Banniere cookies | `frontend/components/CookieBanner.tsx` (nouveau, non monte), `docs/compliance/COOKIES.md` | **Echec technique reel, documente honnetement** : 2 tentatives, un vrai crash d'hydratation React (#418) reproduit deux fois en production. Composant retire du layout par prudence plutot que de risquer un 3e deploiement casse sur une page critique (login). Sans impact reel : cette banniere n'etait de toute facon pas une obligation legale (aucun cookie non essentiel dans l'app) |
