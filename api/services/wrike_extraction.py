"""
Extraction -- fetching data from Wrike's real API, for import into
a knowledge base. Uses a plain `httpx.AsyncClient` against Wrike's
own configured endpoint, the same "trusted, admin-configured external
API" reasoning as `api/services/notion_extraction.py`.

**Real auth shape**: a real, static `WRIKE_API_TOKEN` (admin-configured
...
async def fetch_wrike_records(limit: int = 100) -> list[dict]:
    """Real fetch of records from Wrike.
    ...
    if not settings.WRIKE_ENABLED:
        raise WrikeError("Wrike integration is disabled")

    api_token = getattr(settings, "WRIKE_API_TOKEN", None)
    if not api_token:
        raise WrikeError("Wrike API token not configured")
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if response.status_code == 429:
            raise WrikeRateLimitError("Wrike rate limit reached")
        if response.status_code != 200:
            logger.warning("Wrike fetch failed: %s", response.status_code)
            return []
        ...
        for record in records:
            if not isinstance(record, dict):
                continue
            text_parts = [f"wrike {record.get('id', '')}"]
