...
async def get_user_activity_endpoint(user_id: uuid.UUID, limit: int = Query(default=50, ge=1, le=MAX_PAGE_SIZE), _admin: User = Depends(require_admin), db: AsyncSession = Depends(get_db)):
