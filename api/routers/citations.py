...
Partie 6.1.2/6.1.3/6.1.4 -- every real citation returned here is
enriched with its real, LIVE document name/type, page/section/heading,
AND source URL (`enrich_citation_with_document`/
`enrich_citation_with_location`/`enrich_citation_with_url`, plural
forms for the list endpoints) before being serialized, so a real API
caller always sees the current real values, not only the denormalized
snapshot from citation time. Real, deliberate scope: this enrichment
is NEVER committed back to the database here -- a real GET must stay a
real, honest read with no persistent side effect; only THIS response's
own real, in-memory objects are refreshed."""
...
async def list_citations_by_response_endpoint(
    ...
    return await enrich_citations_with_url(db, citations)
...
async def get_citation_endpoint(
    ...
    return await enrich_citation_with_url(db, citation)
