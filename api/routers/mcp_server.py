...
@router.get("/tools")
async def list_tools_endpoint(_key: OrganizationAPIKey = Depends(require_public_api_scope("mcp:tools"))) -> dict:
    """Real `tools/list`-shaped response -- `{"tools": [...]}`, matching
    `mcp.types.ListToolsResult`'s own real field name."""
    return {"tools": [_tool_to_mcp_shape(tool) for tool in list_tools()]}


@router.post("/tools/{tool_name}/call")
async def call_tool_endpoint(
    tool_name: str, payload: dict[str, Any], _key: OrganizationAPIKey = Depends(require_public_api_scope("mcp:tools")),
) -> dict:
