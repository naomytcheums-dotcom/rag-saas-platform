...
**This is the original, single-tenant RAG pipeline (`src/`) this whole
platform grew from.** Its own honest evaluation methodology and results
below are real and unchanged — see the [root README](../README.md) for
the full, current RAG SaaS Platform (`api/` + `frontend/`) this pipeline's
own retrieval/chunking/embedding logic now backs.
