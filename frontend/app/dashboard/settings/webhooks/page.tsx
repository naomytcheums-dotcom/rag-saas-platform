...
function DeliveryStatusBadge({ delivery }: { delivery: Delivery }) {
  ...
  if (delivery.error || delivery.status_code) {
    return <span className="rounded-full bg-danger-soft px-2 py-0.5 text-xs font-medium text-danger">{delivery.status_code ?? "failed"}</span>;
  }
  return <span className="rounded-full bg-surface-muted px-2 py-0.5 text-xs font-medium text-foreground-muted">pending</span>;
}
...
function WebhookDeliveries({ webhookId }: { webhookId: string }) {
  ...
  if (loading) return <p className="text-xs text-foreground-muted">Loading deliveries…</p>;
  if (deliveries.length === 0) return <p className="text-xs text-foreground-muted">No deliveries yet.</p>;
    ...
    <div className="flex flex-col gap-1.5">
      {deliveries.map((delivery) => (
        <div key={delivery.id} className="flex items-center justify-between rounded-lg bg-background px-2.5 py-1.5 text-xs">
          ...
          <span className="text-foreground-muted">attempt {delivery.attempt}</span>
...
export default function WebhooksPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? String(err.detail) : "Failed to load webhooks");
    } finally {
      setLoading(false);
    }
  }, [org]);
  ...
  async function createWebhook() {
    ...
    try {
      ...
      setError(err instanceof ApiError ? String(err.detail) : "Failed to create webhook");
    } finally {
      setCreating(false);
    }
  }
  ...
  async function testWebhook(id: string) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? String(err.detail) : "Test delivery failed");
    }
  }
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <p className="mt-1 text-sm text-foreground-muted">Get a real HTTP POST whenever something happens in your organization.</p>

      {error && <p className="mt-4 rounded-lg bg-danger-soft px-3 py-2 text-sm text-danger">{error}</p>}

      <div className="mt-6 rounded-xl border border-border bg-surface p-4">
        <h2 className="text-sm font-semibold text-foreground">Add a webhook</h2>
        <input
      ...
      <div className="mt-6">
        <h2 className="mb-2 text-sm font-semibold text-foreground">Configured webhooks</h2>
        {loading ? (
          <p className="text-sm text-foreground-muted">Loading…</p>
        ) : webhooks.length === 0 ? (
          <p className="text-sm text-foreground-muted">No webhooks yet.</p>
        ) : (
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <button type="button" onClick={() => void testWebhook(webhook.id)} className="text-xs font-medium text-accent hover:underline">
                      {testFlash === webhook.id ? "Sent!" : "Test"}
                    </button>
