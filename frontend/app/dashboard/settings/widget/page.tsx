...
export default function WidgetSettingsPage() {
  ...
  async function save() {
    ...
    try {
      ...
      setError(err instanceof ApiError ? String(err.detail) : t("widget.error_save"));
    } finally {
      setSaving(false);
    }
  }
  ...
  async function uploadLogo(file: File) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? String(err.detail) : t("widget.error_logo"));
    }
  }
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="text-xl font-semibold text-foreground">{t("widget.title")}</h1>
      <p className="mt-1 text-sm text-foreground-muted">{t("widget.subtitle")}</p>

      {error && <p className="mt-4 rounded-lg bg-danger-soft px-3 py-2 text-sm text-danger">{error}</p>}
      {savedFlash && <p className="mt-4 rounded-lg bg-success-soft px-3 py-2 text-sm text-success">{t("widget.saved")}</p>}

      <div className="mt-6 flex flex-col gap-5 rounded-xl border border-border bg-surface p-5">
        <div>
          <label htmlFor="widget-name" className="text-sm font-medium text-foreground">{t("widget.name_label")}</label>
        ...
        <div>
          <label htmlFor="widget-welcome" className="text-sm font-medium text-foreground">{t("widget.welcome_label")}</label>
        ...
        <div>
          <p className="text-sm font-medium text-foreground">{t("widget.logo_label")}</p>
          <div className="mt-1 flex items-center gap-3">
            {logoUrl && <Image src={logoUrl} alt={t("widget.logo_alt")} width={40} height={40} unoptimized className="h-10 w-10 rounded-full object-cover" />}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="widget-primary-color" className="text-sm font-medium text-foreground">{t("widget.primary_color")}</label>
          ...
          <div>
            <label htmlFor="widget-header-color" className="text-sm font-medium text-foreground">{t("widget.header_color")}</label>
        ...
        <div>
          <p className="text-sm font-medium text-foreground" id="widget-position-label">{t("widget.position")}</p>
        ...
        <div>
          <p className="text-sm font-medium text-foreground" id="widget-theme-label">{t("widget.theme")}</p>
      ...
      {scriptSnippet && (
        <div className="mt-6 rounded-xl border border-border bg-surface p-4">
          <h2 className="text-sm font-semibold text-foreground">{t("widget.embed_heading")}</h2>
      ...
      {publicKey && (
        <div className="mt-6 rounded-xl border border-border bg-surface p-4">
          <h2 className="text-sm font-semibold text-foreground">{t("widget.preview_heading")}</h2>
          <p className="mt-1 text-xs text-foreground-muted">{t("widget.preview_desc")}</p>
