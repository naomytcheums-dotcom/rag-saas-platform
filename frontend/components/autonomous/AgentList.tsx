...
export function AgentList({ orgId }: { orgId: string }) {
  ...
  if (loading) return <p className="text-sm text-foreground-muted">{t("agent_list.loading")}</p>;
  if (error) return <p className="rounded-lg bg-danger-soft px-3 py-2 text-sm text-danger">{error}</p>;
  if (agents.length === 0) return <p className="rounded-xl border border-border bg-surface p-5 text-sm text-foreground-muted">{t("agent_list.empty")}</p>;
