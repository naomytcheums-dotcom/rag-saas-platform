...
export function AgentCreateForm({ orgId, onCreated }: AgentCreateFormProps) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-border bg-surface p-4">
      <input value={name} onChange={(e) => setName(e.target.value)} aria-label="Nom de l'agent" placeholder="Nom de l'agent" className="rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground" />
      <input value={description} onChange={(e) => setDescription(e.target.value)} aria-label="Description (facultatif)" placeholder="Description (facultatif)" className="rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground" />
      <textarea value={goal} onChange={(e) => setGoal(e.target.value)} aria-label="Objectif — que doit accomplir cet agent ?" placeholder="Objectif — que doit accomplir cet agent ?" rows={3} className="rounded-lg border border-border bg-background px-3 py-2 text-sm text-foreground" />
